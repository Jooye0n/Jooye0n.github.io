...
저장소를 생성했으면, 아래와 같이 clipboard 를 카피하여 로컬에 git clone을 받아준다.
...
### - Homebrew 설치
Mac 사용자의 경우 Jekyll를 포함한 여러 프로그램들의 설치를 용이하게 하기위해 Homebrew 를 먼저 설치해주자.   
> Homebrew 란 오픈소스 기반의 Mac OS용 패키지 매니저다.   
...
### - Bundler 설치
Jekyll 을 설치하기 위해선 gem 명령어를 이용한다. 설치할 많은 gem 의 의존성 관리를 위해 bundler도 설치한다.
...
로컬에서의 확인을 마치면, 아래와 같이 git 저장소에 push 해주고,
```shell
$ git add .
$ git commit -m "Add portfolYOU theme"
$ git push
```   
아래와 같이 정상적으로 빌드되고 나면
![](/assets/img/2024-03-13-Github-블로그-생성하기/actions-update.png)   
   
처음 생성한 내 블로그 url <https://jooye0n.github.io/> 에서도 포스팅 확인이 가능하다.   
