...
2) process 의 context : stact, data, code  
3) OS 의 context : process 하나 당 PCB(Process Control Block) + kernel stack (프로세스마다 있는 운영체제 함수 호출이 이루어질 때의 stack) 가 있다.  
...
> time sharing   
Ready 상태의 process 들이 번갈아가면서 CPU 를 잡는 경우  
...
> Running 중에 디스크에서 무언가를 읽어와야 한다면, process 상태가 Running 에서 Blocked 로 바뀌고 디스크 IO queue 에 들어간다. 디스크는 queue 안에 있는 것을 순서대로 처리한다. 매 작업이 끝나면 Disk controller 가 CPU 에게 요청한 IO 작업이 끝났다는 interrupt 를 건다. 그럼 CPU 제어권이 OS 로 넘어가고 프로세스의 메모리 영역에 데이터를 넘겨준 후 process 상태가 다시 Ready 로 바뀐다. 공유데이터에 접근할 때도 사용중인 process를 blocked 상태로 queue 에 넣는다.  
{: .prompt-tip }
> 어떠한 program 이든 CPU burst 와 I/O burst 를 반복하면서 실행된다. 단, 프로그램의 종류에 따라 빈도가 다르다. 사람과의 interaction 이 많을수록 I/O burst 가 증가한다. I/O 빈도가 많은 program 을 I/O bound job 이라고 하며, CPU 빈도가 많은 program 은 CPU bound job (계산 위주의 job) 이라고 한다. 
여기서의 job 은 process 를 말한다고 볼 수 있겠다.
{: .prompt-tip }

### 3.3 PCB의 개념
PCB란 운영체제가 각 프로세스를 관리하기 위해 프로세스 당 유지하는 정보를 말한다. 크게 아래 네 가지 정보를 가진다.  
1) OS 가 관리상 사용하는 정보 : Process start(ready, running), Process ID, scheduling information, priority
2) CPU 수행 관련 HW값 :PC, R
