...
---
## 2.1 Transfer Object(Value Object) 패턴
...
### 3.1 Servlet 의 특징
1) Multi-Thread 동작
서블릿 컨테이너는 최초의 클라이언트 요청에 대해서 요청된 Servlet 객체를 생성하고 이후부터는 생성된 Servlet을 재사용한다. 이때 클라이언트의 요청에 대해서 Thread 객체를 Thread Pool에서 할당 받아 해당 Thread 객체가 Servlet 프로그램을 수행하는 Multi Thread 형태로 동작한다. 따라서 기존의 CGI 방식에 비 해 효율적으로 동작한다.
2) 플랫폼 독립성
Servlet은 자바를 이용해서 작성되는 클래스이기 때문에 자바 프로그램의 특징을 그대로 적용할 수 있다. 자 바는 플랫폼에 독립적인 언어이므로 한 번 작성된 프로그램은 재컴파일 과정 없이 다양한 플랫폼에서 동일 한 결과를 산출하게 된다. 따라서 자바로 작성되는 서블릿 프로그램은 소스 수정과 재컴파일 과정 없이 다 양한 플랫폼에서 실행될 수 있다
3) 서버 독립성
Servlet은 거의 모든 웹 서버에서 지원되기 때문에 웹 서버와 무관하게 실행될 수 있다.

### 3.2 Servlet의 구현 및 라이프 사이클
1) Servlet 제약사항
Servlet은 자바 클래스로 구현한다. 하지만 일반적인 자바 클래스를 구현하는 것과는 달리 다음과 같은 규칙을 지켜서 구현해야 한다.
- javax.servlet, javax.servlet.http 패키지를 import 해야 한다.
- 반드시 public 클래스로 선언해야 한다.
- 반드시 기본 생성자(Default Contructor)가 있어야 한다.
- Servlet, GenericServet, HttpServlet 클래스 중 하나를 상속해야 한다.
- Servlet 클래스의 Life Cycle과 관련된 메소드를 용도에 맞게 재정의(Overriding) 한다.   

2) Servlet 클래스 구현
Servlet 프로그램을 작성하기 위해서는 HTTP 프로토콜에 최적화 되어있는 HttpServlet을 상속하여 구현해 야 한다. 다음은 Servlet의 일반적인 규칙대로 작성한 간단한 Servlet 프로그램이다.   
```java 
...
public class HelloServlet extends HttpServlet {
    protected void doGet (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.printin ("Hello Servlet!");
        out.close();
    }
}
```
3) Servlet 이름 재지정
Servlet 클래스가 작성되면 어떤 URL 요청에 대해서 해당 Servlet이 생성될지를 Web.xml 파일에 등록해야 한다. 물론 Eclipse에서 Servlet 클래스를 만들었다면 클래스를 만드는 순간 자동으로 web.xml 파일에 등록 될 것이다.
...
위 설정은 /hello.do URL 로 요청 시, hello 라는 이름의 Servlet 이 실행되도록 설정하였다.
따라서 hello 라는 이름으로 등록된 hello.HelloServlet 프로그램이 실행된다.

### 3.3 Servlet 수행흐름
클라이언트의 요청에 대해서 Servlet객체는 다음과 같은 순서로 동작하고 소멸된다. 물론 Servlet 객체 생성부터 소멸에 이르기까지의 모든 관리는 Servlet Container가 담당한다.   
![](/assets/img/2024-09-22-SPRING-기본/spring_7.png)   

 
> `destroy() ` 메소드 호출 시점
Servlet 프로그램은 많게는 수 많은 유저가 하나의 Servlet 프로그램을 같이 사용하기 때문에 한 명의 유 저에 의해 프로그램이 종료되면 다른 유저는 Servlet 프로그램을 이용할 수 없다. 따라서 destroy() 메소 드는 Servlet Container가 종료되거나 장시간 해당 Servlet 이 호출되지 않을 경우 자동으로 컨테이너에 의해 호출된다.
{: .prompt-info }  
