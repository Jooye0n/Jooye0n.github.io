...
GitHub Pages에서 제공하는 기능을 사용하기 위해선, Repository name을 **_`{사용자명}.github.io`_** 형태로 설정해야 한다.  
...
```shell
$ brew install rbenv        //(24.03.10 기준) 최신 버전으로 설치
...
> Ruby에 gem 설치 중 `You don't have write permissions for the /usr/bin directory.` 오류가 뜰땐,  
> 쓰기 권한이 있는 경로로 변경해준 후 쉘 설정 파일에 rbenv PATH를 추가하자.  
> 사용하는 쉘에 따라서 bash는 .bashrc zsh는 .zshrc에 각각 아래 내용을 추가하면 된다.  
```shell
$ brew install rbenv ruby-build     //rbenv 설치
$ rbenv install 3.1.4               //Ruby 설치
  rbenv global 3.1.4
```  
```shell
$ vi ~/.zshrc
...
```shell
$ gem install bundler jekyll       //jekyll 설치
```
